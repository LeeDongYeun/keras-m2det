...
from ..models.m2det import m2det_bbox
...
def create_models(backbone_m2det, num_classes, weights, multi_gpu=0,
                  freeze_backbone=False, lr=1e-5, config=None):
    """ Creates three models (model, training_model, prediction_model).

    Args
        backbone_m2det : A function to call to create a m2det model with a given backbone.
    ...
    if multi_gpu > 1:
        ...
        with tf.device('/cpu:0'):
            model = model_with_weights(backbone_m2det(num_classes, num_anchors=num_anchors, modifier=modifier), weights=weights, skip_mismatch=True)
    ...
    prediction_model = m2det_bbox(model=model, anchor_params=anchor_params)
...
def parse_args(args):
    ...
    parser.add_argument('--backbone',         help='Backbone model used by retinanet.', default='vgg16', type=str)
